    ...
    devs: true,
     run: async (client, message, args) => {
       ...
       if(user && type === "coins") {
         ...
         message.channel.send({content: `> **Done removed \`${amount}\` coins from ${user}**`}).then(msg => {
           setTimeout(function() {
             msg.delete();
           }, 3000);
         });
         ...
         message.channel.send({content: `> **Done removed \`${amount}\` level from ${user}**`}).then(msg => {
           setTimeout(function() {
             msg.delete();
           }, 3000);
         });
         ...
         message.channel.send({content: `**Done removed \`${amount}\` likes from ${user}**`}).then(msg => {
           setTimeout(function() {
             msg.delete();
           }, 3000);
         });
